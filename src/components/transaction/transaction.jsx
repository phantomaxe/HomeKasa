...
export default function Properties() {
  ...
  const [selectCategory, setSelectCategory] = useState('UPCOMING');
  ...
  }, [filterOption, allData, historyData, selectCategory]);
  ...
  return (
    <Paper className={classes.cardSection}>
      ...
      <Box display="flex" className={classes.categorySection}>
        {selectTag.map((value, index) => (
          <Box
            key={value}
            className={clsx(
              { [classes.categoryBox]: true },
              { [classes.activeCategoryBox]: selectCategory === value }
            )}
          ...
          >
            <Typography
              className={clsx(
                ...
                { [classes.activeCategory]: selectCategory === value }
              )}
            >
              {value}
            </Typography>
            <Divider
              className={clsx(
                ...
                { [classes.activeDivider]: selectCategory === value }
              )}
            />
          </Box>
        ))}
      ...
      <Box display="flex" className={classes.filterSection}>
        ...
        <Box textAlign="left" className={classes.marginLeft50}>
          <Typography className={classes.greyColor}>NEXT</Typography>
          <FormControl
            variant="outlined"
            size="small"
            color="secondary"
            ...
            {types.map((type) => (
              <Typography
                ...
                className={clsx(
                  ...
                  {
                    [classes.activeTypeStyle]: filterOption.type.includes(type),
                  }
                )}
              >
                {_.capitalize(type)}
        ...
        <Table stickyHeader aria-label="incomeExpenditureTable">
          <TableHead>
            <TableRow>
              ...
              >
                {selectCategory === 'UPCOMING' ? 'DUE DATE' : 'PAID ON'}
